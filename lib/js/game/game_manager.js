...
class GameManager {
    ...
    loadLevel(onfinished) {
        let self = this;
        OBJ.downloadMeshes({
            'player': 'env/models/low_poly_man.obj', // located in the models folder on the server
            //'bird': 'env/models/bird.obj',
            'floor': 'env/models/floorbig.obj'
        }, function(meshes) {
            self.player.initModelData(meshes.player);
            self.player.material.texture = new Image();
            self.player.material.texture.src = 'env/textures/low_poly_man_5eyes.png';

            //self.bird.initModelData(meshes.bird);
            //self.bird.material.texture = new Image();
            //self.bird.material.texture.src = 'env/textures/bird.png';

            self.floor.initModelData(meshes.floor);
            self.floor.material.texture = new Image();
            self.floor.material.texture.src = 'env/textures/floor64x.png';
        });
    }
