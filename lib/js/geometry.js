...
rgti.class.Geometry = class Geometry {
  ...
  process (specs) {
    if (specs) {
      ...
      v = this.vertices && JSON.parse("[" + this.vertices.slice(1).join() + "]");
      t = this.textures && JSON.parse("[" + this.textures.slice(1).join() + "]");
      n = this.normals && JSON.parse("[" + this.normals.slice(1).join() + "]");
